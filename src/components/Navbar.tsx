  ...
  useEffect(() => {
    const fetchAppearance = async () => {
      try {
        ...
        console.error('Failed to fetch navbar appearance:', error);
      }
    };
    fetchAppearance();
  }, []);
          ...
          <div className="hidden md:flex space-x-8">
            {navItems.map((item) => (
              ...
              >
                {item.label}
                <span
                  className={`absolute bottom-0 left-0 right-0 h-0.5 transition-all duration-300 ${isActive(item.path) ? 'w-full' : 'w-0 group-hover:w-full'
                    }`}
